...
/**
 * Authenticate against Traccar Server via REST API and provide Authentication
 * for Spring Security and Spring MVC.
 * <p>
 * The Authentication creates an instantiated Open API Client with an authorized
 * User for a session. The method also sets the three possible Traccar roles for
 * Spring authorities management.
 * <p>
 * see: www.marcobehler.com/guides/spring-security
 * <p>
 * Note that this class can be used in plain Spring projects and does not
 * require Vaadin or other frontend designs (may be moved to a separate
 * project)!
 */
public class TraccarAuthenticationProvider implements AuthenticationProvider {
	 ...
	 * Traccar User Entity as Principal and attach its authenticated and authorized
	 * Traccar instance to the Authentication which is managed in the normal Spring
	 * life cycle and destroyed properly at the end of a session.
	 */
	@Override
...
//		TODO define explicit vaadin user, in case Traccar API is unreachable .. implement UserDetailsService ?
...
//		credentials / password will be null-ed internally and cannot be recalled
	...
	/**
	 * Instantiate Open API client to manage the connection to the Traccar backend
	 * during the complete session. Note that the user has not been authenticated
	 * yet and must be done in a consecutive step.
	 */
	...
	/**
	 * Authenticate user and password against Traccar Backend and start a valid
	 * session user after authorization.
	 * 
	...
	private User authenticateHttpBasic(DefaultApi traccarApi, String username, String password) {
		...
		HttpBasicAuth basicAuth = (HttpBasicAuth)
				traccarApi.getApiClient().getAuthentication("basicAuth");
...
//			AuthenticationCredentialsNotFoundException
	...
	/**
	 * Evaluate the Traccar User to grant Spring permissions. Only three roles
	 * available as described at www.traccar.org/documentation/user-management
	 */

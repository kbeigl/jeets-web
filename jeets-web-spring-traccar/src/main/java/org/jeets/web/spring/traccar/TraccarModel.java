...
/**
 * Provides the original Traccar Model as a starting point for rapid development
 * of Controllers and Views.
 * <p>
 * Technically a wrapper for each Traccar ORM and Cache for server responses.
 * <p>
 * Internally it will hold the original Traccar Model and keep it in sync via
 * Traccar API. It also serves as a cache for previously retrieved model details
 * in order to save redundant requests. (add .refresh method/s) The instance is
...
//	check SpringMVC concepts: ModelAndView = Model, View, ModelAttributes etc.
	...
	/**
	 * This authentication only works after the TraccarAuthenticationProvider has
	 * authenticated a user against the Traccar Server and who is authorized for
	 * Spring Security. As a consequence: <br>
	 * DO NOT INJECT THE TraccarModel <br>
	 * BEFORE THE TraccarAuthenticationProvider HAS DONE HIS JOB!
	 * <p>
	 * If this method ever fails it might make sense to call this (public!) method
...
//			attached to the Authentication before it can be retrieved here.
			traccarApi = (DefaultApi) 
					TraccarAuthentication.getAuthentication().getDetails();
//			catch and handle CastExep ?
			System.out.println("authenticated Traccar User " +
...
//	TODO display ${devices} in a html view as reference to @ModelAttribute
	/**
	 * The Device list is created for session use and can be used to lookup each
	 * Device (by different search methods).
	 * <p>
	 * The Device itself represents the entry point to the ORM. For example you
	 * lookup Events via Device.getEvents(filtered).
	 */
	 ...
	 * <p>
	 * Note the device list has to be set by a set*Devices() method.
	 */
...
//		model.addAttribute("test", new Integer(12));
		...
		=>	 *   userId - Standard users can use this only with their own _userId_ (optional)
			 ...
			 */
			devices = traccarApi.devicesGet(null,TraccarAuthentication.getTraccarUser().getId(),null,null);
//			return traccarApi.devicesGet(null,TraccarAuthentication.getTraccarUser().getId(),null,null);
